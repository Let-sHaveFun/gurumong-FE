...
import type { PlaceDetailData } from '../api/getDetail.query';
import { PLACE_LIST } from '@/shared/constants/badge-images';
...
const serviceIds = PLACE_LIST.map((place) => place.id);

export function PlaceDetailView({ data }: { data: PlaceDetailData }) {
  ...
  const isServiceId = serviceIds.includes(placeId!);
      ...
      <Flex gap="$100" flexDirection="column" padding="$000">
          ...
          <div className="overflow-y-auto h-[400px] mt-[120px] px-10">
            <div className="overflow-y-auto mb-[100px]">
              <Text typography="body2" className={cn(typography.body2, 'whitespace-pre-line')}>
                {/* {legendText} */}
                {isServiceId ? data.script : '서비스를 준비중이예요!'}
              </Text>
            </div>
          </div>
        </section>
        ...
        {isServiceId && (
          <FixedBottom className="p-0">
            <AudioPlayer onCompleteAudio={onCompleteAudio} audioUrl={data.url ?? ''} />
          </FixedBottom>
        )}
